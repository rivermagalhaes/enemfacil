...
const MATERIAS: Record<string, Materia[]> = {
  ...
  ITA: [
    { id: "matematica", label: "Matemática", emoji: "📐", trilha: "matematica" },
    { id: "fisica",     label: "Física",     emoji: "⚡", trilha: "fisica" },
    { id: "quimica",    label: "Química",    emoji: "🧪", trilha: "quimica" },
    { id: "portugues",  label: "Português",  emoji: "📖", trilha: "portugues" },
    { id: "ingles",     label: "Inglês",     emoji: "🇺🇸", trilha: "ingles" },
  ],
  IME: [
    { id: "matematica", label: "Matemática", emoji: "📐", trilha: "matematica" },
    { id: "fisica",     label: "Física",     emoji: "⚡", trilha: "fisica" },
...
export default function VestibularHub() {
  ...
  const acoes = [
    { id: "quiz", emoji: "🧠", titulo: "Quiz", desc: "Questões reais por matéria", cor: "#0057FF", bg: "#E6EEFF", onClick: () => navigate(`/quiz/vestibular/${vestibular.toUpperCase()}`) },
    { id: "simulado", emoji: "📝", titulo: "Simulado", desc: "Prova completa cronometrada", cor: "#7C3AED", bg: "#F5F3FF", onClick: () => navigate(`/simulado`) },
    { id: "agente", emoji: "🤖", titulo: "Prof. IA", desc: `Especialista em ${v.nome}`, cor: v.cor, bg: v.bg, onClick: () => navigate(`/agentes/${vestibular.toUpperCase()}`) },
    { id: "ranking", emoji: "🏆", titulo: "Ranking", desc: "Top alunos deste vestibular", cor: "#D97706", bg: "#FFFBEB", onClick: () => navigate(`/ranking/${vestibular.toUpperCase()}`) },
  ];
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh", background: CORES.bg }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px 90px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
          {acoes.map(a => (
            <button key={a.id} onClick={a.onClick} style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", padding: "16px 14px", borderRadius: 16, background: CORES.bgCard, border: `1.5px solid ${a.cor}22`, cursor: "pointer", textAlign: "left", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: a.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, marginBottom: 10 }}>{a.emoji}</div>
        ...
        <p style={{ fontSize: 11, fontWeight: 700, color: CORES.textSub, textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 12px" }}>Estudar por matéria</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
          {materias.map((m, i) => (
            <div key={i} style={{ display: "flex", gap: 8 }}>
              <button onClick={() => navigate(`/quiz/vestibular/${vestibular.toUpperCase()}?area=${m.id}`)} style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", borderRadius: 12, background: CORES.bgCard, border: `1px solid ${CORES.border}`, cursor: "pointer", textAlign: "left" }}>
              ...
              <button onClick={() => navigate(`/trilha/${vestibular.toUpperCase()}/${m.trilha}`)} style={{ width: 52, height: 52, borderRadius: 12, flexShrink: 0, background: v.bg, border: `1.5px solid ${v.cor}33`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", gap: 2 }}>
        ...
        <div style={{ background: v.bg, borderRadius: 12, padding: "12px 14px", border: `1px solid ${v.cor}22` }}>
          ...
          <p style={{ fontSize: 12, color: CORES.textSub, margin: 0, lineHeight: 1.6 }}>Use o <strong>Quiz</strong> para praticar questões reais, o <strong>Prof. IA</strong> para tirar dúvidas específicas e a <strong>Trilha</strong> para estudar o conteúdo de forma estruturada.</p>
        </div>
      </div>
      <BottomNav />
    </div>
