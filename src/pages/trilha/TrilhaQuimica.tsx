...
export default function TrilhaQuimica() {
  ...
  const corVest = COR_VEST[vestUpper] ?? "#7C3AED";

  useEffect(() => { if (user?.id) carregarProgresso(); }, [user?.id]);

  async function carregarProgresso() {
    const [{ data: prog }, { data: mats }] = await Promise.all([
      supabase.from("trilha_progresso").select("unidade_id, status, xp_ganho").eq("user_id", user!.id).eq("vestibular", vestUpper).eq("materia", "quimica"),
      supabase.from("materiais").select("id,titulo,descricao,tipo,url,topic").eq("vestibular", vestUpper).eq("materia", "quimica").eq("ativo", true).order("criado_em", { ascending: false }),
    ]);
  ...
  async function concluirUnidade(unidadeId: string, xp: number) {
    if (!user?.id) return;
    await supabase.from("trilha_progresso").upsert({ user_id: user.id, vestibular: vestUpper, materia: "quimica", unidade_id: unidadeId, status: "concluido", xp_ganho: xp, atualizado_em: new Date().toISOString() }, { onConflict: "user_id,vestibular,materia,unidade_id" });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh", background: CORES.bg }}>
      {mapaMentalAberto && (
        <MapaMental unidades={UNIDADES} titulo="Química — Ensino Médio" onClose={() => setMapaMentalAberto(false)}
          onSelecionarUnidade={(id) => { const u = UNIDADES.find(u => u.id === id); if (u) setUnidadeAberta(u); }} progresso={progresso} />
      )}

      {unidadeAberta && (
        <ModalUnidade unidade={unidadeAberta} vestibular={vestUpper} materia="quimica"
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px 90px" }}>
        ...
        {materiais.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: CORES.textSub, textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 10px" }}>
              📁 Materiais {vestUpper}
            </p>
                ...
                return (
                  <a key={m.id} href={m.url} target="_blank" rel="noreferrer"
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {m.descricao && <p style={{ fontSize: 11, color: CORES.textSub, margin: "2px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.descricao}</p>}
                      {m.topic && <span style={{ fontSize: 10, color: corVest, background: `${corVest}15`, padding: "1px 6px", borderRadius: 4, fontWeight: 600 }}>{m.topic}</span>}
                    </div>
