...
const UNIDADES = [
  {
    id: "pre-historia",
    numero: 1,
    titulo: "Pré-História e Antiguidade",
    emoji: "🏛️",
    cor: "#B45309",
    bg: "#FFF8E6",
    topic: null,
    topicos: ["Surgimento do ser humano", "Mesopotâmia e Egito", "Grécia Antiga", "Roma Antiga"],
    xp: 50,
  },
  {
    id: "idade-media",
    ...
    bg: "#F9FAFB",
    topic: "idade-media",
    topicos: ["Feudalismo", "Igreja Católica", "Cruzadas", "Renascimento"],
    xp: 80,
  },
  {
    id: "idade-moderna",
    ...
    bg: "#E6EEFF",
    topic: "idade-moderna",
    topicos: ["Grandes Navegações", "Reforma Protestante", "Absolutismo", "Mercantilismo"],
    xp: 100,
  },
  {
    id: "revolucoes",
    numero: 4,
    titulo: "Revoluções e Iluminismo",
    emoji: "🔦",
    cor: "#7C3AED",
    bg: "#F3F0FF",
    topic: "revolucoes",
    topicos: ["Iluminismo", "Revolução Francesa", "Revolução Industrial", "Independências americanas"],
    xp: 100,
  },
  {
    id: "brasil-colonial",
    ...
    topicos: ["Colonização portuguesa", "Ciclos econômicos", "Escravidão", "Quilombos"],
    xp: 100,
  },
  {
    id: "brasil-imperio",
    ...
    topicos: ["Independência do Brasil", "Período imperial", "Proclamação da República", "República Velha"],
    xp: 100,
  },
  {
    id: "guerras-mundiais",
    ...
    topicos: ["1ª Guerra Mundial", "Nazifascismo", "2ª Guerra Mundial", "Holocausto"],
    xp: 100,
  },
  {
    id: "brasil-republica",
    numero: 8,
    titulo: "Brasil República Moderna",
    emoji: "🏙️",
    cor: "#0ea5e9",
    bg: "#e0f2fe",
    topic: "brasil-republica",
    topicos: ["Era Vargas", "Democracia e Ditadura Militar", "Redemocratização", "Brasil contemporâneo"],
    xp: 100,
  },
  {
    id: "geografia-fisica",
    ...
    topicos: ["Relevo e clima", "Hidrografia", "Biomas brasileiros", "Questões ambientais"],
    xp: 80,
  },
  {
    id: "geografia-humana",
    numero: 10,
    titulo: "Geografia Humana e Geopolítica",
    emoji: "🗺️",
    cor: "#8b5cf6",
    bg: "#f5f3ff",
    topic: "geografia-humana",
    topicos: ["Urbanização", "Globalização", "Blocos econômicos", "Conflitos geopolíticos"],
    xp: 100,
  },
  {
    id: "filosofia-sociologia",
    numero: 11,
    titulo: "Filosofia e Sociologia",
    emoji: "🧠",
    cor: "#06b6d4",
    bg: "#ecfeff",
    topic: "filosofia-sociologia",
    topicos: ["Filosofia antiga e moderna", "Contratualistas", "Marx, Weber e Durkheim", "Cidadania e direitos"],
    xp: 100,
  },
];
...
interface QuestaoOption {
  question_id: string; option_index: number; label: string;
}

function ModalQuestoes({
  unidade, vestibular, onClose, onConcluir,
}: {
  unidade: typeof UNIDADES[0]; vestibular: string;
  ...
  async function carregarQuestoes() {
    const { data: qs } = await supabase
      .from("questions").select("id, question, explanation, answer_index, difficulty, ano")
      .eq("vestibular", vestibular).eq("topic", unidade.topic).limit(5);
  ...
  function proxima() {
    if (idx + 1 >= questoes.length) { setFim(true); onConcluir(); }
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 480, maxHeight: "90dvh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
        <div style={{ background: unidade.cor, padding: "14px 16px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{unidade.emoji}</div>
          <div style={{ flex: 1 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
          ...
          {!loading && !unidade.topic && (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 600, color: CORES.text, margin: "0 0 6px" }}>Unidade introdutória</p>
              <p style={{ fontSize: 13, color: CORES.textSub, marginBottom: 20 }}>Esta unidade apresenta os conceitos base. Questões aparecerão ao longo da trilha.</p>
...
export default function TrilhaHumanas() {
  ...
  const corVest =
    vestUpper === "ITA" ? "#003D80" : vestUpper === "IME" ? "#1a3a6e" :
    vestUpper === "FUVEST" ? "#8B0000" : vestUpper === "UNICAMP" ? "#005C97" :
    vestUpper === "UNB" ? "#006400" : "#B45309";
  ...
  async function carregarProgresso() {
    const { data } = await supabase.from("trilha_progresso")
      .select("unidade_id, status, xp_ganho")
      .eq("user_id", user!.id).eq("vestibular", vestUpper).eq("materia", "humanas");
  ...
  async function concluirUnidade(unidadeId: string, xp: number) {
    ...
    await supabase.from("trilha_progresso").upsert({
      user_id: user.id, vestibular: vestUpper, materia: "humanas",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh", background: CORES.bg }}>
      {unidadeAberta && (
        <ModalQuestoes unidade={unidadeAberta} vestibular={vestUpper}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 14px 90px" }}>
        ...
        <div style={{ marginTop: 16, background: "#fef9ec", borderRadius: 12, padding: "12px 14px", border: "1px solid #fcd34d" }}>
          ...
          <p style={{ fontSize: 12, color: CORES.textSub, margin: 0, lineHeight: 1.6 }}>
            Complete as unidades em ordem. Cada unidade tem questões reais filtradas por tema. Ganhe XP ao concluir!
          </p>
        </div>
      </div>
      <BottomNav />
