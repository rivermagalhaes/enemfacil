...
// Configuração visual de cada olimpíada
const CONFIG: Record<string, { nome: string; emoji: string; cor: string; bg: string; desc: string }> = {
  // Colégios Militares do Tocantins
  OQCMTO: { nome: "1ª Olimpíada de Química dos C.M. Tocantins",    emoji: "🧪", cor: "#1a3a6e", bg: "#eef2ff", desc: "Colégios Militares do Tocantins · 2026" },
  OMCMTO: { nome: "1ª Olimpíada de Matemática dos C.M. Tocantins", emoji: "📐", cor: "#1a3a6e", bg: "#eef2ff", desc: "Colégios Militares do Tocantins · 2026" },
  OFCMTO: { nome: "1ª Olimpíada de Física dos C.M. Tocantins",     emoji: "⚡", cor: "#1a3a6e", bg: "#eef2ff", desc: "Colégios Militares do Tocantins · 2026" },
  OPCMTO: { nome: "1ª Olimpíada de Português dos C.M. Tocantins",  emoji: "📝", cor: "#1a3a6e", bg: "#eef2ff", desc: "Colégios Militares do Tocantins · 2026" },
  // IFTO
  OQIFTO: { nome: "1ª Olimpíada de Química do IFTO",               emoji: "🔬", cor: "#065C37", bg: "#EDFAF3", desc: "Instituto Federal do Tocantins · 2026" },
  // Estadual
...
export default function OlimpiadaHub() {
  const { id = "OQCMTO" } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { user, profile } = useAuth();
  const cfg = CONFIG[id.toUpperCase()] ?? CONFIG.OQCMTO;
  ...
  const acoes = [
    ...(prova ? [{
      id: "prova", emoji: "📝", titulo: "Prova Oficial",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {acoes.map(a => (
            <button key={a.id} onClick={a.onClick}
              ...
              <div style={{ width: 52, height: 52, borderRadius: 14, background: (a as any).destaque ? "rgba(255,255,255,0.2)" : a.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, flexShrink: 0 }}>
                {a.emoji}
              </div>
              ...
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke={(a as any).destaque ? "#fff" : a.cor} strokeWidth="2"><path d="M6 4l4 4-4 4"/></svg>
            </button>
          ))}
        </div>
