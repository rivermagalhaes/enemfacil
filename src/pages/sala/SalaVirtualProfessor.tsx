...
export default function SalaVirtualProfessor() {
  ...
  const [erroSimulado, setErroSimulado] = useState<string | null>(null);
  ...
  async function carregarMateriaisBiblioteca() {
    const { data } = await supabase
      ...
      .eq("ativo", true).eq("criado_por", user!.id).order("criado_em", { ascending: false });
  ...
  async function criarSimulado() {
    ...
    if (!error && data) {
      ...
      setErroSimulado(`Erro ao criar simulado: ${error.message}`);
      console.error("Erro criarSimulado:", error);
    }
      ...
      {modalSimulado && (
        <>
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 1001, background: "#0f172a", borderRadius: "20px 20px 0 0", padding: 20, maxHeight: "92dvh", overflowY: "auto", boxShadow: "0 -8px 40px rgba(0,0,0,0.4)" }}>
            ...
            {erroSimulado && (
              <div style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.4)", borderRadius: 10, padding: "10px 14px", marginBottom: 10 }}>
                <p style={{ fontSize: 12, color: "#f87171", margin: 0 }}>❌ {erroSimulado}</p>
              </div>
            )}
