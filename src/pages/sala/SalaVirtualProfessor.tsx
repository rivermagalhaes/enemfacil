...
export default function SalaVirtualProfessor() {
  ...
  async function carregarAssignments() {
    ...
    if (data && data.length > 0) {
      ...
      const { data: subs } = await supabase
        ...
        .select("assignment_id, acertos, total")
  ...
  async function buscarQuestoes() {
    ...
    const q = supabase.from("questions").select("id, question, answer_index, explanation, topic, area").limit(50);
  ...
  async function criarSimulado() {
    ...
    const { data, error } = await supabase.from("assignments").insert({
      ...
      descricao: formSimulado.descricao || null,
      professor_id: user!.id,
      sala_virtual_id: formSimulado.sala_virtual_id,
      ...
      questoes_data: questoesFiltradas,
      ativo: true,
    }).select().single();
  ...
  async function criarSala() {
    ...
    const { data, error } = await supabase.from("salas_virtuais").insert({
      ...
      modo: form.modo, tempo_limite_min: form.tempo_limite_min ? parseInt(form.tempo_limite_min) : null,
        ...
        <div style={{ padding: "16px 20px 80px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 20 }}>
            {[
              ...
              {
                val: submissoesDetalhe.length > 0
                  ? `${Math.round(submissoesDetalhe.reduce((s: number, r: any) => s + (r.total > 0 ? r.acertos / r.total * 100 : 0), 0) / submissoesDetalhe.length)}%`
                  : "—",
                lbl: "Média geral", emoji: "🎯"
              },
            ].map(m => (
    ...
    <div style={{ minHeight: "100dvh", background: "#0a0f1e", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#065C37,#0A7C4B)", padding: "14px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button
            onClick={() => tabAtiva === "salas" ? setModalCriar(true) : setModalSimulado(true)}
          ...
          <>
            {salaAtiva && (
              <div style={{ background: "linear-gradient(135deg,#065C37,#0A7C4B)", borderRadius: 16, padding: 16, marginBottom: 20 }}>
                ...
                <button onClick={() => encerrarSala(salaAtiva)} style={{ width: "100%", padding: "12px 0", background: "rgba(239,68,68,0.2)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                  ⏹ Encerrar sala
                </button>
              </div>
            )}
                  ...
                  return (
                    <div key={sala.id} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 14, padding: "14px 16px", border: "1px solid rgba(255,255,255,0.07)" }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        {sala.status === "ativa" && (
                          <button onClick={() => { setSalaAtiva(sala); carregarParticipantes(sala.id); carregarStats(sala.id); }} style={{ flex: 1, padding: "9px 0", background: "rgba(78,206,154,0.15)", color: "#4ece9a", border: "1px solid rgba(78,206,154,0.3)", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>👁 Ver ao vivo</button>
                        )}
                        ...
                        <button onClick={() => excluirSala(sala.id)} style={{ padding: "9px 14px", background: "rgba(239,68,68,0.1)", color: "#ef4444", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>🗑</button>
                      </div>
                    </div>
                  );
                })}
        ...
        <>
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 1001, background: "#0f172a", borderRadius: "20px 20px 0 0", padding: 20, maxHeight: "90dvh", overflowY: "auto", boxShadow: "0 -8px 40px rgba(0,0,0,0.4)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 6, marginBottom: 14 }}>
              {MATERIAS.map(m => (
                <button key={m.id} onClick={() => setForm(f => ({ ...f, materia: m.id }))} style={{ padding: "8px 4px", borderRadius: 10, border: "none", cursor: "pointer", background: form.materia === m.id ? "rgba(10,124,75,0.3)" : "rgba(255,255,255,0.05)", outline: form.materia === m.id ? "1.5px solid #0A7C4B" : "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
            ...
            <button onClick={criarSala} disabled={salvando} style={{ width: "100%", padding: "14px 0", background: salvando ? "rgba(255,255,255,0.1)" : "linear-gradient(90deg,#065C37,#0A7C4B)", color: "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: salvando ? "not-allowed" : "pointer" }}>
      ...
      {modalSimulado && (
        <>
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 1001, background: "#0f172a", borderRadius: "20px 20px 0 0", padding: 20, maxHeight: "92dvh", overflowY: "auto", boxShadow: "0 -8px 40px rgba(0,0,0,0.4)" }}>
                ...
                {salas.map(s => (
                  <button key={s.id} onClick={() => setFormSimulado(f => ({ ...f, sala_virtual_id: s.id }))}
                    style={{ padding: "10px 14px", borderRadius: 10, border: "none", cursor: "pointer", textAlign: "left",
                      background: formSimulado.sala_virtual_id === s.id ? "rgba(10,124,75,0.2)" : "rgba(255,255,255,0.05)",
                      outline: formSimulado.sala_virtual_id === s.id ? "1.5px solid #0A7C4B" : "none" }}>
            ...
            <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", margin: "0 0 6px" }}>📅 Data e hora de liberação</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
              <div>
                <p style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", margin: "0 0 4px" }}>Data</p>
                <input type="date" value={formSimulado.data_liberacao}
                  onChange={e => setFormSimulado(f => ({ ...f, data_liberacao: e.target.value }))} style={{ ...INPUT, colorScheme: "dark" }} />
              </div>
              <div>
                <p style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", margin: "0 0 4px" }}>Hora</p>
                <input type="time" value={formSimulado.hora_liberacao}
                  onChange={e => setFormSimulado(f => ({ ...f, hora_liberacao: e.target.value }))} style={{ ...INPUT, colorScheme: "dark" }} />
              </div>
            </div>

            {/* Tempo limite */}
            <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", margin: "0 0 6px" }}>⏱ Tempo limite (min)</p>
            <input type="number" placeholder="Sem limite" value={formSimulado.tempo_limite_min}
              onChange={e => setFormSimulado(f => ({ ...f, tempo_limite_min: e.target.value }))} style={{ ...INPUT, marginBottom: 20 }} />
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 16, marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                ...
                <button onClick={buscarQuestoes} disabled={buscandoQuestoes}
                  style={{ padding: "10px 16px", background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 300, overflowY: "auto" }}>
                {questoesImportadas.map(q => {
                  ...
                  return (
                    <button key={q.id} onClick={() => {
                      ...
                      background: sel ? "rgba(10,124,75,0.2)" : "rgba(255,255,255,0.04)",
                      outline: sel ? "1.5px solid #0A7C4B" : "1px solid rgba(255,255,255,0.07)" }}>
                ...
                {questoesImportadas.length === 0 && (
                  <p style={{ fontSize: 12, color: "#64748b", textAlign: "center", padding: 16 }}>Clique em "Buscar" para carregar questões</p>
                )}
              </div>
            </div>
