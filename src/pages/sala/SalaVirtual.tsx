...
export default function SalaVirtual() {
  ...
  // Fases: entrada | aguardando | quiz | resultado
  const [fase, setFase] = useState<"entrada" | "aguardando" | "quiz" | "resultado">("entrada");
  ...
  // Polling sala status
  useEffect(() => {
    if (!sala || fase === "entrada") return;
    const interval = setInterval(async () => {
      const { data } = await supabase
        .from("salas_virtuais").select("*").eq("id", sala.id).single();
      if (!data) return;
      if (data.status === "ativa" && fase === "aguardando") {
  ...
  async function entrarNaSala() {
    ...
    const { data: salaData } = await supabase
      .from("salas_virtuais")
      .select("*")
      .eq("codigo", codigoInput.toUpperCase().trim())
      .single();
    ...
    setSala(salaData);
    if (salaData.status === "ativa") {
      await carregarQuestoes(salaData);
      setFase("quiz");
    } else {
      setFase("aguardando");
    }
  ...
  async function carregarQuestoes(s: Sala) {
    ...
    if (!data) return;
    const qs: Questao[] = data.map((q: any) => ({
      ...q,
  ...
  async function carregarRanking(salaId: string) {
    ...
    const lista: RankingItem[] = Object.entries(mapa)
  ...
  if (fase === "entrada") return (
    <div style={{ minHeight: "100dvh", background: "linear-gradient(135deg,#0a0f1e,#1a1a2e)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <p style={{ fontSize: 22, fontWeight: 800, color: "#fff", margin: "0 0 4px" }}>Sala Virtual</p>
      <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", margin: "0 0 32px" }}>Digite o código fornecido pelo professor</p>
      ...
      <div style={{ width: "100%", maxWidth: 360 }}>
        ...
        />
        {erro && <p style={{ fontSize: 12, color: "#ef4444", textAlign: "center", margin: "0 0 10px" }}>{erro}</p>}
        <button
          onClick={entrarNaSala}
          disabled={entrando || !codigoInput}
          style={{
            width: "100%", padding: "14px 0", borderRadius: 12, border: "none",
            background: entrando || !codigoInput ? "rgba(255,255,255,0.1)" : "linear-gradient(90deg,#0057FF,#0ea5e9)",
            color: "#fff", fontSize: 15, fontWeight: 700, cursor: entrando || !codigoInput ? "not-allowed" : "pointer",
          }}
        >
          {entrando ? "Entrando..." : "Entrar na sala →"}
        </button>
        <button onClick={() => navigate("/")} style={{ width: "100%", marginTop: 10, padding: "12px 0", borderRadius: 12, border: "none", background: "transparent", color: "rgba(255,255,255,0.4)", fontSize: 13, cursor: "pointer" }}>
          Voltar
        </button>
      </div>
    </div>
  ...
  if (fase === "aguardando" && sala) return (
    <div style={{ minHeight: "100dvh", background: "linear-gradient(135deg,#0a0f1e,#1a1a2e)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ fontSize: 48, marginBottom: 12, animation: "pulse 2s infinite" }}>
        {MATERIA_EMOJI[sala.materia] ?? "🎯"}
      </div>
      ...
      <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 14, padding: 16, width: "100%", maxWidth: 360 }}>
        <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", margin: "0 0 10px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
          👥 {participantes.length} na sala
        </p>
      ...
      <div style={{ minHeight: "100dvh", background: "#0a0f1e", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: "linear-gradient(135deg,#0057FF,#0ea5e9)", padding: "12px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
            ...
            {tempoRestante !== null && (
              <span style={{
                fontSize: 13, fontWeight: 800, color: tempoRestante < 30 ? "#ff4444" : "#fff",
                background: "rgba(0,0,0,0.2)", borderRadius: 99, padding: "3px 10px",
              }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {questoes.map((_, i) => (
              <div key={i} style={{ flex: 1, height: 4, borderRadius: 99, background: i < qIdx ? "#4ece9a" : i === qIdx ? "#fff" : "rgba(255,255,255,0.3)" }} />
            ))}
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 24px" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 14, padding: "14px 16px", marginBottom: 16, border: "1px solid rgba(255,255,255,0.08)" }}>
            {q.topic && (
              <span style={{ fontSize: 10, fontWeight: 700, background: "#0057FF30", color: "#60a5fa", borderRadius: 99, padding: "2px 8px", display: "inline-block", marginBottom: 8 }}>
                {q.topic}
              </span>
            )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {q.options.map(opt => {
              ...
              if (selecionada && resp === undefined) { bg = "rgba(0,87,255,0.2)"; border = "1.5px solid #0057FF"; }

              return (
                <button
                  key={opt.option_index}
                  onClick={() => responder(opt.option_index)}
                  disabled={!!resp}
          ...
          {resp !== undefined && (
            <div style={{ marginTop: 14, background: resp.correta ? "rgba(78,206,154,0.1)" : "rgba(239,68,68,0.1)", borderRadius: 12, padding: "12px 14px", border: `1px solid ${resp.correta ? "#4ece9a40" : "#ef444440"}` }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: resp.correta ? "#4ece9a" : "#ef4444", margin: "0 0 4px" }}>
                {resp.correta ? "✅ Correto!" : "❌ Incorreto"}
              </p>
              {q.explanation && <p style={{ fontSize: 12, color: "#aaa", margin: 0, lineHeight: 1.5 }}>{q.explanation}</p>}
              <button
                onClick={proximaQuestao}
                style={{ marginTop: 10, width: "100%", padding: "10px 0", background: "linear-gradient(90deg,#0057FF,#0ea5e9)", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}
              >
      ...
      <div style={{ minHeight: "100dvh", background: "linear-gradient(135deg,#0a0f1e,#1a1a2e)", padding: 20 }}>
        <div style={{ textAlign: "center", paddingTop: 32, marginBottom: 24 }}>
          <div style={{ fontSize: 52, marginBottom: 8 }}>
            {minhaPos === 1 ? "🥇" : minhaPos === 2 ? "🥈" : minhaPos === 3 ? "🥉" : "🏆"}
          </div>
          <p style={{ fontSize: 22, fontWeight: 800, color: "#fff", margin: "0 0 4px" }}>
            {minhaPos === 1 ? "Você ganhou!" : `${minhaPos}º lugar`}
          </p>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", margin: 0 }}>
            {acertos} de {total || questoes.length} acertos
          </p>
        </div>
        ...
        <button
          onClick={() => navigate("/")}
          style={{ width: "100%", padding: "14px 0", background: "linear-gradient(90deg,#0057FF,#0ea5e9)", color: "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: "pointer" }}
        >
          Voltar ao início
        </button>
      </div>
