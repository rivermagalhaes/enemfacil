...
export default function ProfessorDashboard() {
  ...
  return (
    <div style={{ minHeight:"100dvh",background:CORES.bg,fontFamily:"system-ui,sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background:"linear-gradient(135deg,#065C37,#0A7C4B)",padding:"16px 16px 24px",position:"relative",overflow:"hidden" }}>
        <div style={{ position:"absolute",inset:0,opacity:0.15,background:"radial-gradient(circle at 80% 20%, #4ade80 0%, transparent 60%), radial-gradient(circle at 20% 80%, #065C37 0%, transparent 60%)",pointerEvents:"none" }} />
        <div style={{ position:"relative" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:16 }}>
            <button onClick={()=>navigate("/")} style={{ width:32,height:32,borderRadius:"50%",background:"rgba(255,255,255,0.2)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="#fff" strokeWidth="2"><path d="M10 3L5 8l5 5"/></svg>
            </button>
            <div style={{ flex:1 }}>
              <p style={{ fontSize:9,color:"rgba(255,255,255,0.6)",fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",margin:"0 0 2px" }}>Painel do Professor</p>
              <p style={{ fontSize:18,fontWeight:800,color:"#fff",margin:0,lineHeight:1.2 }}>
                👨‍🏫 {(profile as any)?.nome_professor || (profile as any)?.nome || "Professor"}
              </p>
              {(profile as any)?.nome_escola && (
                <p style={{ fontSize:11,color:"rgba(255,255,255,0.7)",margin:"2px 0 0" }}>🏫 {(profile as any).nome_escola}</p>
              )}
            </div>
            <span style={{ fontSize:10,background:"rgba(255,255,255,0.2)",color:"#fff",borderRadius:99,padding:"4px 12px",fontWeight:700,flexShrink:0 }}>PROFESSOR</span>
          </div>
          <div style={{ display:"flex",gap:8 }}>
            {[
              ...
              { emoji:"🏫", label:"Salas" },
              { emoji:"🖨️", label:"Impressão" },
            ].map((item,i) => (
              <div key={i} style={{ flex:1,background:"rgba(255,255,255,0.12)",borderRadius:10,padding:"8px 4px",textAlign:"center",backdropFilter:"blur(10px)" }}>
                <p style={{ fontSize:16,margin:"0 0 2px" }}>{item.emoji}</p>
                <p style={{ fontSize:9,color:"rgba(255,255,255,0.8)",margin:0,fontWeight:600 }}>{item.label}</p>
              </div>
            ))}
          </div>
        </div>
